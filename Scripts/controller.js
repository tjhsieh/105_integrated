...
function controller() 
{
    ...
    load_example("./Xml/Dougong_32.xml");
    ...
    function read_XML_shape(xml) 
	{
        ...
        $(xml).find("shape").each(function (i) {
			...
			setTimeout(function(){ set_porgress(i, alldata);shape_grammar_controller.open_example(symbol); block_switch = ui_manager.get_block_switch(); },9500);
			//給讀取時間，預防模型出錯
	...
	function doOnLoad(url)
	{
			...
			myTree.load(url);
			setTimeout(function(){Load_list();},3500);//預設所有checkbox打勾
